/* 
This script handles user preferences by storing and retrieving user data from a user's browser cookie to 
save space on the user's computer. 
*/
function showModalDropdownOptions(obj) { 
  // moved dropdown options to a modal to prevent the options from overlapping the page
...
function settingsSwitch(setting) {
  ...
  $("#settingsBlock").find(".setting").each((index, elem) => {
    ...
    if ($(elem).data("setting-call-function")) callableFunctionsFromJqueryData[
      $(elem).data("setting-call-function")
    ]();
  });
}
function changeSettings() {
  localStorage.setItem("oracles_appSettings", JSON.stringify(Object.fromEntries(new URLSearchParams(
    $("#settingsBlock").serialize()
  ))));
